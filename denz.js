...
try {
        ...
        mess = {
			...
			success: 'Berhasil',
			error: {
				stick: 'Video Nya Ga Bisa Diakses Bro',
				Iv: 'Itu Bukan Stiker Pea',
                api: 'Error'
			},
			only: {
				group: 'Cuma Bisa Di Group jing',
				ownerG: 'Emang Bapa Lu Yang Punya Grup?',
				ownerB: 'Lu Siapa, Kok Nyuruh Gua?',
				admin: 'Cuma Admin Yang Bisa Coeg',
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© ChooXD_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JnO3nmhJnNq42MNCIKXycl', "groupName": `${NamaBot}`, "footerText": "*_© ChooXD_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
		...
		message: {
			"productMessage": {
				"product": {
					...
					"title": `Acexyz Botz By ChooXD`, //Kasih namalu 
					"description": `© ChooXD`, 
					"currencyCode": "USD",
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `Acexyz Botz By ChooXD`, //Kasih namalu
...
const finv = {
	...
	"message": {
		"groupInviteMessage": {
			...
			"inviteCode": `Acexyz Botz By ChooXD`,
			"groupName": `ChooXD`, 
            "caption": `${tampilUcapan} ${pushname}`, 
            'jpegThumbnail': dfrply
		}
...
const ftex = {
	 ...
	 message: { 
		"extendedTextMessage": {
                 "text": `Acexyz Botz By ChooXD`,
                 "title": `Fabio Nicholas A.M (ChooXD)`,
                 'jpegThumbnail': dfrply
                        }
	                  } 
                     }
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"© ChooXD",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6281357620824`}}})
		}
		const simir = (teks) => {
...
reply(String(e))
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				reply('Link Group Terdeteksi, Auto Kick!')
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© ChooXD_*", [{buttonId: 'github', buttonText: {displayText: 'Github'}, type: 1},{buttonId: 'tiktok', buttonText: {displayText: 'TikTok'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'github'){
reply('http://github.com/ChooXD')
}
if(isButton == 'tiktok'){
reply('http://tiktok.com/Xero2.0')
}
   ...
   const sotoy = [
	    ...
	    if (isCmd && !isGroup) {console.log(color('|Group|', 'greenyellow'), color(moment(mek.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(`${command} [${args.length}]`, 'cyan'), color(`${pushname}`, 'orange'), color(`${sender}`, 'deeppink'))}
	    if (!command) {console.log(color('|Command|', 'greenyellow'), color(moment(mek.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(cmd, 'cyan'), color(`${pushname}`, 'orange'), color(`${sender}`, 'deeppink'))}
...
{buttonId:`menusimple`,buttonText:{displayText:'🤍SIMPLE MENU🤍'},type:1}
]
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_©ChooXD_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`menusimple`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '©ChooXD'
       ownerJid = "6281357620824@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
├ _Creator : *ChooXD*
...
break    
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					rate = body.slice(1)
					const ra =['0,1','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39']
